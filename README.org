...
These approximations are only valid for $E_{\alpha,\beta}(-x)$, where $\alpha \le \beta$ & $x \ge 0$
In math: ($R_{\alpha,\beta}^{m,n}(x) \approx E_{\alpha,\beta}(-x), x \ge 0 & \alpha < \beta$)
